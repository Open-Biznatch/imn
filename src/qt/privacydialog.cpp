                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zIMN ought to be enough for anybody." - Bill Gates, 2017
    ui->zIMNpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzIMNSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzIMN->setEnabled(false);
        ui->pushButtonMintzIMN->setToolTip(tr("zIMN is currently disabled due to maintenance."));

        ui->pushButtonSpendzIMN->setEnabled(false);
        ui->pushButtonSpendzIMN->setToolTip(tr("zIMN is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zIMNpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzIMN_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zIMN is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zIMN...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zIMN in ") +
...
void PrivacyDialog::on_pushButtonSpendzIMN_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zIMN is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zIMN
        sendzIMN();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zIMN
    sendzIMN();
}

void PrivacyDialog::on_pushButtonZImnControl_clicked()
{
    ZImnControlDialog* zImnControl = new ZImnControlDialog(this);
    zImnControl->setModel(walletModel);
    zImnControl->exec();
}

void PrivacyDialog::setZImnControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzImnSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzIMN()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Imn Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zIMNpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zIMN is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " IMN </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zIMN</b>";
    ...
    // use mints from zImn selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZImnControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZImnControlDialog::GetSelectedMints();
    }
    ...
    // Clear zimn selector in case it was used
    ZImnControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zImn Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Imn, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zImn Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zIMN </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zIMN "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zIMN "));
    ui->labelzIMNAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzIMNSyncStatus->setVisible(fShow);
}
