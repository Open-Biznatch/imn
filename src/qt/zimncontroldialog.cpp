...
#include "ui_zimncontroldialog.h"
...
std::list<std::string> ZImnControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZImnControlDialog::listMints;

ZImnControlDialog::ZImnControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZImnControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZImnControlDialog::~ZImnControlDialog()
{
    delete ui;
}

void ZImnControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZImnControlDialog::updateList()
{
...
void ZImnControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZImnControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZImn_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZImnControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZImnControlDialog::GetSelectedMints()
{
...
void ZImnControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
